<style>
    ...
    .mini-extrato { margin-top: 8px; padding-top: 8px; border-top: 1px solid #334155; font-size: 11px; color: #94a3b8; }
    .mini-extrato-item { display: flex; justify-content: space-between; margin-bottom: 2px; }
</style>
...
<div id="metaPage" style="display:none;">
    ...
    <div class="card">
        <h3>📑 Extrato Detalhado por Conta</h3>
...
function registrarGanho() {
    ...
    const distribuicao = m.despesas.map(d => ({
        nome: d.nome,
        valor: liq * d.percentual
    }));

    if (!m.historicoObjetos) m.historicoObjetos = [];
    m.historicoObjetos.push({ 
        bruto: bruto, 
        gas: vGas, 
        diz: vDiz, 
        liquido: liq, 
...
function calcularAcumuladoConta(nomeConta) {
    const m = metas[metaAtual];
    return m.historicoObjetos.reduce((s, aporte) => {
        const item = (aporte.distribuicao || []).find(dist => dist.nome === nomeConta);
...
function renderizarHistoricoMeta() {
    ...
    [...(m.historicoObjetos || [])].reverse().forEach((obj, i) => {
        ...
        h += `<div class="historico-item"><div class="flex"><small>${obj.data}</small><button class="btn-del-mini" onclick="apagarLancamento(${rIdx})">X</button></div>
            ...
            <div style="text-align:right; color:var(--green)"><b>Líquido: R$ ${obj.liquido.toFixed(2)}</b></div></div>`;
    });
    document.getElementById('historicoGanhos').innerHTML = h || 'Vazio.';
}

function renderizarExtratoReal() {
    const m = metas[metaAtual];
    let h = '';
    (m.despesas || []).forEach(d => {
        ...
        h += `<div class="historico-item" style="border-left: 4px solid ${d.quitada ? 'var(--green)' : 'var(--blue)'}">
            ...
            <div class="mini-extrato">
                ${htmlAportes || 'Nenhum aporte registrado.'}
                <div class="flex" style="margin-top:5px; padding-top:5px; border-top:1px dashed #334155; color: ${falta === 0 ? 'var(--green)' : 'var(--red)'}">
                    <span>FALTA:</span> <b>R$ ${falta.toFixed(2)}</b>
                </div>
            </div>
        </div>`;
    });
    document.getElementById('extratoContas').innerHTML = h || 'Crie uma despesa para ver o extrato.';
}
